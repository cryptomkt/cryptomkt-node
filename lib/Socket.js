...
class Socket {
    ...
    _setupWorkerEvents() {
        ...
        this.socket.on('balance', (data) => {
            ...
            this.eventBus.emit('balance-fees', this.deepClone(balanceFeesData.data));
        });

        this.socket.on('balance-delta', (deltas) => {
            if (this.debugging) {
            ...
            _.forEach(deltas, (delta, index, collection) => {
                ...
                if (index === collection.length - 1) {
                    ...
                    this.eventBus.emit('balances', this.deepClone(balancesData.data));
                }
            });

        });

        this.socket.on('operated', (data) => {
            if (this.debugging) {
                console.log('[' + new Date().toUTCString() + '] operated');
            }
            this.operatedData = data;
            this.eventBus.emit('operated', this.deepClone(this.operatedData.data));
        });

        this.socket.on('operated-delta', (deltas) => {
            if (this.debugging) {
        ...
        this.socket.on('open-book', (data) => {
            ...
            this.eventBus.emit('open-book', this.deepClone(clean));
        });

        this.socket.on('open-book-delta', (delta) => {
            if (this.debugging) {
        ...
        this.socket.on('historical-book', (data) => {
            ...
            this.eventBus.emit('historical-book', this.deepClone(result));
        });

        this.socket.on('historical-book-delta', (delta) => {
            if (this.debugging) {
            ...
            if (_.has(this.historicalBookData, stockId)) {
                if (this.historicalBookData[stockId].to_tx !== delta.from_tx) {
                    this.socket.emit("historical-book", {
                        stockId: stockId
                    });
                    return;
                }
        ...
        this.socket.on('candles', (data) => {
            ...
            this.eventBus.emit('candles', this.deepClone(result));
        });

        this.socket.on('candles-delta', (delta) => {
            if (this.debugging) {
        ...
        this.socket.on('board', (data) => {
            ...
            this.boardData = data;
            this.eventBus.emit('ticker', this.deepClone(boardData.data));
        });

        this.socket.on('board-delta', (delta) => {
            if (this.debugging) {
